...
import { FaTrophy, FaCalendarAlt, FaMapMarkerAlt, FaUserShield, FaEdit, FaTrash, FaList } from 'react-icons/fa';
...
export function ManageGames() {
  ...
  const handleDelete = async (id: string) => {
    const confirm = window.confirm("Tem certeza que deseja excluir este jogo?");
    if (confirm) {
      await deleteGame(id);
  ...
  if (loading) return <div className="manage-games-page"><p style={{color: 'white'}}>Carregando jogos...</p></div>;
        ...
        {editingGame ? (
          <form className="edit-game-form" onSubmit={handleUpdate}>
            ...
            <label>
              ...
              <div className="input-wrap">
                ...
                <input 
                  ...
                  onChange={(e) => setEditingGame({...editingGame, dateTime: e.target.value})} 
                  required 
                />
              </div>
            </label>
            ...
            <div className="edit-actions">
              ...
              <button className="cancel-btn" type="button" onClick={() => setEditingGame(null)}>Cancelar</button>
            </div>
          </form>
        ) : (
              ...
              games.map((game) => (
                <div className="game-item" key={game.id}>
                  <div className="game-info">
                    <h4>{game.title}</h4>
                    <span className="game-detail"><FaUserShield /> {game.opponent}</span>
                    <span className="game-detail"><FaCalendarAlt /> {new Date(game.dateTime).toLocaleString('pt-BR')}</span>
                    <span className="game-detail"><FaMapMarkerAlt /> {game.location}</span>
                  </div>
                  <div className="game-actions">
                    <button className="icon-btn edit" onClick={() => setEditingGame(game)} title="Editar">
                      <FaEdit />
                    </button>
                    <button className="icon-btn delete" onClick={() => handleDelete(game.id)} title="Excluir">
                      <FaTrash />
                    </button>
                  </div>
                </div>
